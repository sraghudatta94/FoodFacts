...
/*Array to Retrive Data */
var myarray = [];
/*Pre defined Country Values*/
...
/*Sugar,Salt,Fat,Protein,Carbohydrate Dummy Arrays*/
...
lineReader.on('line', function(line) {
    myarray = line.split(new RegExp(/,(?=(?:(?:[^"]*"){2})*[^"]*$)/)); //To Avoid charecters in btw data
    /*Searching for country and calculating its salt and sugar value*/
    if (myarray[33].includes("Netherlands")) {
        var tempSugar = parseFloat(myarray[102]);
        var tempSalt = parseFloat(myarray[116]);

        if (isNaN(tempSugar)) {} else {

            sugar[0] = sugar[0] + tempSugar;
        }
        if (isNaN(tempSalt)) {} else {
    ...
    //To calculate Fat,Protein,Carbohydrate values for regions
...
//Assigns values to array of objects
lineReader.on('close', function() {

    for (let i = 0; i < 8; i++) { //forjson1.json
    ...
    for (let i = 0; i < 3; i++) { //for data.json
...
//Creating Json file using write stream
